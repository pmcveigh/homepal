...
def compute_next_due_date(schedule: RecurringSchedule, completed_on: date) -> date:
    if schedule.recurrence_type in {RecurrenceType.EVERY_N_DAYS, RecurrenceType.AFTER_COMPLETION_N_DAYS}:
    ...
    if schedule.recurrence_type == RecurrenceType.EVERY_N_MONTHS:
        ...
        year = completed_on.year + ((completed_on.month - 1 + months) // 12)
        month = (completed_on.month - 1 + months) % 12 + 1
        day = min(completed_on.day, 28)
        return date(year, month, day)
    if schedule.recurrence_type == RecurrenceType.FIXED_ANNUAL_DATE:
        month = schedule.month or completed_on.month
        day = schedule.day or completed_on.day
